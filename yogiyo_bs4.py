...
region = "서울특별시"
post_num = 140201
address = "강원도 속초시 설악동 699"
category = ["한식", "치킨", "피자양식", "중국집", "한식", "일식돈까스", "족발보쌈", "야식", "분식", "카페디저트", "편의점마트","1인분주문"]
...
for c in category:
    ...
    for i in range(len(store_list)):
        store_list2 = driver.find_elements_by_xpath("//div[@class='item clearfix']")
        print(store_list2)

        driver.execute_script("arguments[0].click();", store_list2[i])
        #store.send_keys(Keys.COMMAND + "\n")
        ...
        for review in reviews:
            if "전" in review.find(class_="review-time ng-binding").get_text() or "어제" in review.find(class_="review-time ng-binding").get_text() or (2019 <= int(review.find(class_="review-time ng-binding").get_text().split("년")[0]) and 3 <= int(review.find(class_="review-time ng-binding").get_text().split(" ")[1][0])):
                if "어제" in review.find(class_="review-time ng-binding").get_text() or "전" in review.find(class_="review-time ng-binding").get_text():
                    year = 2021
                ...
                row = pd.DataFrame([(address.split(" ")[0], address.split(" ")[1],c, store_name, year, month, len(overall), taste, quantity, delivery, comment)],
